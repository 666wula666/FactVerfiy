...
# 4. Output the JSON in a single line without any line breaks.
...
# """
decompose_prompt = """
Your task is to extract **valuable, verifiable atomic claims** from the given text.

The output must be a JSON object with a single key "claims" (no more than 10 claims).  
Its value should be a list of concise, context-independent factual statements.

Guidelines:
1. Each claim must express **a single, concrete fact** that can be independently verified.  
2. Claims should focus on **valuable information** — data, events, results, relationships, or measurable attributes.  
   - Avoid trivial or generic descriptions that cannot be verified (e.g., opinions, emotions, vague judgments).  
3. Each claim should be **concise (under 15 words)** and **self-contained** — it should make sense without additional context.  
4. Avoid ambiguous pronouns or references like “he,” “she,” “it,” “the company.”  
   Always use explicit entities or names.  
5. Generate at least one claim per sentence, splitting long sentences (over 50 words) into multiple atomic claims.  
6. Output must be a **single-line JSON** without line breaks.
