"""
CloudswaySearchClient 异步测试脚本
使用异步方法测试 CloudswaySearchClient 的基本功能
"""
...
class CloudswaySearchClient:
    ...
    async def search(self, query, count=10, sites=None, include_raw_content=False):
        """执行异步搜索查询"""
        # URL encode query
        ...
        try:
            ...
            # 尝试不同的认证方法
            return await self._search_with_retry(endpoint_url, params, query, include_raw_content)
        except Exception as e:
            logger.error(f"搜索出错: {str(e)}")
            return self._empty_result(query)

    async def _search_with_retry(self, endpoint_url, params, query, include_raw_content):
        """使用重试逻辑的内部异步搜索方法"""
        # 尝试不同的认证方法
        auth_methods = [
            # 方法1: Bearer 认证
            ...
            try:
                ...
                async with self.session.get(
                    ...
                    if response.status == 200:
                        logger.info(f"搜索成功，状态码: {response.status}")
...
async def main():
    ...
    try:
        ...
        for query in queries:
            tasks.append(process_query(client, query))
